...
2.  **File Structure:**
    * `/data`: (Link to the Kaggle dataset or include a sample).
    * `/notebooks`: `Final_Project_Air_Quality_India.ipynb` (Ensure all cells are run and plots are visible).
    * `/presentation`: `Project_Presentation.pdf` (Convert your .pptx to .pdf for direct browser viewing).
